...
NAME="index.js"
DIR=`/root/mirrorbits-downloads $(/bin/readlink -f $0)`
APP=${DIR}/${DESC}/${NAME}
...
running() {
	node_modules/.bin/forever list 2>/dev/null | grep ${APP} 2>&1 >/dev/null
    return $?
}

start_server() {
	node_modules/.bin/forever start ${APP} 2>&1 >/dev/null
	return $?
}

stop_server() {
	node_modules/.bin/forever stop ${APP} 2>&1 >/dev/null
	return $?
}
